...
export const activityRecords = sqliteTable(
  "activity_records",
  {
    ...
    userActivityId: text("user_activity_id").references(
      () => userActivities.id,
    ),
  ...
  (table) => [
    check(
      `activity_or_user_activity_check`,
...
export const userMealIngredients = sqliteTable(
  ...
  (table) => [
    check(
      `food_or_user_food_check`,
      sql`(${table.foodId} IS NOT NULL) != (${table.userFoodId} IS NOT NULL)`,
    ),
  ],
);

export const consumptionRecords = sqliteTable(
  "consumption_records",
  {
    id: text().primaryKey().notNull(),
    foodId: text("food_id").references(() => foods.id),
...
export const activitysRelations = relations(activities, ({ many }) => ({
...
export const activityRecordsRelations = relations(
  ...
  ({ one }) => ({
    ...
    userActivity: one(userActivities, {
      fields: [activityRecords.userActivityId],
...
export const userMealIngredientsRelations = relations(
  ...
  ({ one }) => ({
    food: one(foods, {
      fields: [userMealIngredients.foodId],
      references: [foods.id],
    }),
    userFood: one(userFoods, {
      fields: [userMealIngredients.userFoodId],
      references: [userFoods.id],
    }),
    userMeals: one(userMeals, {
      fields: [userMealIngredients.userMealId],
      references: [userMeals.id],
    }),
  }),
);

export const consumptionRecordsRelations = relations(
  consumptionRecords,
  ({ one }) => ({
    food: one(foods, {
      fields: [consumptionRecords.foodId],
      references: [foods.id],
    }),
    userFood: one(userFoods, {
      fields: [consumptionRecords.userFoodId],
      references: [userFoods.id],
    }),
    userMeal: one(userMeals, {
      fields: [consumptionRecords.userMealId],
      references: [userMeals.id],
    }),
  }),
);
