...
export default function FoodPopupCreateFoodContent({ onDone }: Props) {
  ...
  const [ingredients, setIngredients] = useState<{ food: FoodItem; quantityInGrams: number }[]>([]);
    ...
    {!ingredientMode ? (
      <>
        <TextField
          ...
          onChangeText={(name) => setCreatedMeal(({
            ...createdMeal,
            name: name
          }))}
          numberOfLines={8}
        ...
        <TextField
          ...
          onChangeText={(notes) => setCreatedMeal(({
            ...createdMeal,
            notes: notes
          }))}
          multiline={true}
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }} >
          ...
          <AppText
            type='title-small'
          >{`${ingredients.reduce((sum, ing) => sum + Math.floor(ing.quantityInGrams * ing.food.caloricValue / 100), 0)
            } kcal`}</AppText>
          <IconButton
            icon={{
              name: 'add',
          ...
          renderItem={({ item }) => (
            <>
              ...
              >
                ...
                />
                <AppText
                  style={{ textAlignVertical: 'center', marginBottom: 4, flex: 1 }}
                  type='body-large'
